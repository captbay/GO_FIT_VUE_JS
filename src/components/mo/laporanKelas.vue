    ...
    <v-main class="list">
        ...
        <v-card>
            <v-list-item style="padding: 16px;">
                <v-list-item-content>
                    <v-list-item-title class="headline"> Laporan Aktivitas Kelas </v-list-item-title>
                </v-list-item-content>
            </v-list-item>
            <v-card-title>
        ...
        <v-card>
            <v-data-table :headers="headers" :items="aktivitasCLASS" :search="search">
            </v-data-table>
        </v-card>
...
export default {
    data() {
        return {
            ...
            headers: [
                ...
                { text: "Instruktur", value: "nama_instruktur" },
                { text: "Jumlah Peserta", value: "jumlah_peserta" },
                { text: "Jumlah Libur", value: "jumlah_libur_kelas" },
            ],
    ...
    methods: {
        ...
        getData(bulan, tahun) {
            axios.get(Api.BASE_URL + `/aktivitasKelasPerTahun/${bulan}/${tahun}`, {
                headers: {
                    'Accept': 'application/json',
                    'Authorization': 'Bearer ' + $cookies.get("SESSION")
                }
            }).then((response) => {
                this.aktivitasCLASS = response.data.data;
        ...
        getBulanTahun() {
            axios.get(Api.BASE_URL + "/howManyMonthYearInClassBooking", {
                headers: {
                    'Accept': 'application/json',
                    'Authorization': 'Bearer ' + $cookies.get("SESSION")
                }
        ...
        cetakPDF(bulan, tahun) {
            if (tahun == null || bulan == null) {
                ...
                axios.get(Api.BASE_URL + `/aktivitasKelasPerTahunPDF/${bulan}/${tahun}`, {
                    responseType: 'blob',
                    headers: {
                        'Accept': 'application/json',
                        'Authorization': 'Bearer ' + $cookies.get("SESSION")
                    ...
                    fileLink.setAttribute('download', `aktivitasCLASS Bulan ${bulan} - ${tahun}.pdf`);
